...
from ppcgrader.args import add_remote_argument, parse_args, clang_compiler, command_type, command_from_name, gcc_compiler, nvcc_compiler
...
def cli(config: Config):
    ...
    for command_name in command_type(config.gpu)(args.command):
        command_class = command_from_name(command_name, config.gpu)
        command = command_class(command_name, config)
        passed = command.exec(compiler=compiler,
                              reporter=reporter,
